document.addEventListener("DOMContentLoaded", function () {
    ...
    const tipoInstEl = document.getElementById('tipo_institucion');
    if (tipoInstEl) {
        ...
        // Evitar error: "An invalid form control ... is not focusable" por select oculto y required
        if (tipoInstEl.hasAttribute('required')) tipoInstEl.removeAttribute('required');
        const form = tipoInstEl.closest('form');
        if (form) {
            form.addEventListener('submit', function(e){
                // Validación manual: exige selección distinta a vacío
                if (!tipoInstEl.value) {
                    e.preventDefault();
                    alert('Por favor, seleccione el tipo de institución.');
                    const ui = tipoInstEl.parentElement && tipoInstEl.parentElement.querySelector('.choices');
                    if (ui) { ui.focus(); }
                }
            });
        }
