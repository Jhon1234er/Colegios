...
$totalEstudiante = (new Aprendiz())->contarAprendices();
$totalProfesores = (new Facilitador())->contarFacilitadores();
$totalMaterias   = (new AreaDeConocimiento())->contarCursos();
$colegios        = (new Colegio())->obtenerTodos();

// Helper
function formatearNombreColegio($nombre) {
    $nombre = mb_convert_case($nombre, MB_CASE_TITLE, "UTF-8");
    $nombre = preg_replace_callback('/(\s|,)\s*([A-Za-z])\.([A-Za-z])\.?/u', function($m) { return $m[1] . strtoupper($m[2]) . '.' . strtoupper($m[3]); }, $nombre);
    $nombre = preg_replace_callback('/\(([a-zA-Z]{2,})\)/u', function($m) { return '(' . strtoupper($m[1]) . ')'; }, $nombre);
    return $nombre;
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <!-- Bienvenida -->
        <div class="div9">
            ...
            <?php 
              ...
              $nombre_corto = htmlspecialchars((explode(' ', $n1)[0] ?? '').' '.(explode(' ', $a1)[0] ?? ''));
            ?>
            <p><strong><?= $bienvenida ?></strong> <?= $nombre_corto ?></p>
    ...
    <div id="professor-details-sidebar" class="student-details-sidebar" style="display: none;">
        <div class="details-header">
            ...
            <button id="close-prof-details" class="close-prof-details-btn close-details-btn">&times;</button>
        </div>
    ...
    <!-- Contenedor para detalles del colegio -->
    <div id="school-details-sidebar" class="student-details-sidebar" style="display: none;">
        <div class="details-header">
            <h2>Detalles del Colegio</h2>
            <button id="close-school-details" class="close-details-btn">&times;</button>
        </div>
        <div id="school-details-content" class="student-details-empty">
            <div class="empty-state">
                <p>Selecciona un colegio para ver sus detalles</p>
            </div>
        </div>
    </div>
  ...
  <div class="modal-dialog modal-lg">
        ...
        window.alert = function(msg){
          try {
            if (typeof mostrarNotificacionTemporal === 'function') {
              var m = String(msg||'');
              var type = /error|falló|fallo|no pudo|invalid/i.test(m) ? 'error' : ( /éxito|exito|ok|listo/i.test(m) ? 'success' : 'info');
    ...
    <script>
      ...
      (function(){
        ...
        var nombre = <?= json_encode(explode(' ', trim($usuario['nombres']))[0] ?? '') ?>;
        var RETURN_THRESHOLD = 120000; // 2 minutos

        var RETURN_GREETINGS = [
          '¡Hola de nuevo!',
        ...
        function saludoPorHora(){
          try {
            ...
            var parts = new Intl.DateTimeFormat('es-CO', { hour: 'numeric', hour12: false, timeZone: tz }).formatToParts(new Date());
            var hourPart = parts.find(p=>p.type==='hour');
        ...
        function siguienteSaludo(){
          var idx = Number(localStorage.getItem('greetIdx')||0);
          var txt = RETURN_GREETINGS[idx % RETURN_GREETINGS.length];
          localStorage.setItem('greetIdx', String((idx+1) % RETURN_GREETINGS.length));
          return txt;
        }

        function setSaludoInicial(){
          var last = Number(localStorage.getItem('lastActiveTs')||0);
          var now = Date.now();
          var awayMs = now - last;
        ...
        window.addEventListener('beforeunload', function(){ localStorage.setItem('lastActiveTs', String(Date.now())); });
      })();
    </script>
...
$(function(){
  function setupReportModal(suf, downloadPage){
    ...
    function buildThead($table, dias){
      const cols = ['Documento','Nombre','Ficha']
        .concat(dias.map(function(d){ return nombreDiaEs(d); }))
        .concat(['Jornada','Estado']);
      const tr = '<tr>'+cols.map(function(h){ return '<th>'+h+'</th>'; }).join('')+'</tr>';
    ...
    function doPreview(){
      ...
      $.post('/?page=preview_v2', payload, function(html){
        ...
        const diasStr = ($meta.attr('data-dias')||'').split(',').filter(function(s){ return s !== ''; });
        $meta.remove();
  ...
  if (typeof flatpickr !== 'undefined') {
    function makeFpOptions(side, colegioSelector, modalSelector, shiftX, moreTransparent, alignTargetSelector){
      return {
        dateFormat: 'Y-m-d',
    ...
    const multiOpts = function(side, colegioSelector, modalSelector){
      return {
        mode: 'multiple',
        dateFormat: 'Y-m-d',
        locale: (window.flatpickr && flatpickr.l10ns && flatpickr.l10ns.es) ? flatpickr.l10ns.es : 'es',
        ...
        onOpen: function(_, __, inst){
          ...
          const modalRect = modal.getBoundingClientRect();
          const target = document.querySelector(colegioSelector) || inst.input;
          ...
          cal.style.top = String(Math.max(8, top)) + 'px';
          const maxLeft = modalRect.width - cRect.width - 8;
          left = Math.max(8, Math.min(left, maxLeft));
          cal.style.left = String(left) + 'px';
        },
        onChange: function(){
    ...
    flatpickr('#weekStartPDF',  multiOpts('PDF', '#selectColegioPDF',  '#modalReportesPDF .modal-content'));
    flatpickr('#weekStartExcel',multiOpts('Excel','#selectColegioExcel','#modalReportesExcel .modal-content'));
    ...
    (function(){
      ...
      ['#weekStartPDF','#weekEndPDF','#weekStartExcel','#weekEndExcel'].forEach(function(sel){
