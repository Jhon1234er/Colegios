...
?>
<link rel="stylesheet" href="/css/AreaDeConocimiento/tabla.css">

<div class="tabla-cursos">
    <div class="card-widget">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;">
        <h2 class="section-title" style="margin:0;">Listado de Cursos</h2>
        <a href="/?page=cursos&action=crear" class="btn btn-primary">+ Registrar Cursos</a>
      </div>
      ...
      <form method="GET" action="" style="display:flex;gap:10px;align-items:flex-end;margin-top:14px;flex-wrap:wrap;">
        ...
        <div>
          ...
          <a href="/?page=cursos" class="<?= $isFiltering ? 'btn btn-warning btn-size' : 'btn btn-light btn-size' ?>">Limpiar</a>
        </div>
      </form>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px;">
        ...
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <div>
            <?php $prev = max(1, $pageNum - 1); $next = min($totalPages, $pageNum + 1); $qs = http_build_query(array_filter(['page'=>'cursos','q'=>$q??'','estado'=>$estado??'','linea'=>$linea??''])); ?>
              ...
              <?php foreach ($materias as $m): ?>
                <tr>
                  ...
                  <td class="acciones-celda">
                    <a href="/?page=cursos&action=editar&id=<?= urlencode($m['id']) ?>" class="btn-accion btn-warning" style="padding: 6px 10px;">
                      <i class="fas fa-edit"></i> Editar
                    </a>
                    <?php if (($m['estado'] ?? 'activa') === 'suspendida'): ?>
                      <a href="/?page=cursos&action=activar&id=<?= urlencode($m['id']) ?>" class="btn-accion btn-activar btn-confirm" style="padding: 6px 10px;" data-action="activar">
                        <i class="fas fa-check"></i> Activar
                      </a>
                    <?php else: ?>
                      <a href="/?page=cursos&action=suspender&id=<?= urlencode($m['id']) ?>" class="btn-accion btn-danger btn-confirm" style="padding: 6px 10px;" data-action="suspender">
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  document.querySelectorAll('.btn-confirm').forEach(function(btn){
    btn.addEventListener('click', function(e){
      ...
      const msgSuspend = 'Al suspender, se desactivarán todas la funciones de la Ficha generada con este curso ¿Deseas continuar?';
      const msgActivar = 'Se activará nuevamente este curso. ¿Deseas continuar?';
