            ...
            <?php foreach ($resultados as $resultado): ?>
                <div class="result-card"><?php if ($filtro === 'colegio'): ?>
                        ...
                        <div class="result-actions">
                            <button type="button" class="btn btn-primary btn-ver-detalles-colegio" data-id="<?= htmlspecialchars($resultado['id']) ?>">Ver Detalles</button>
                        </div>
                    ...
                    <?php elseif ($filtro === 'profesor'): ?>
                        <div class="result-header">
                            <?php
                              ...
                              // Normalizar tipo de contrato (puede venir como tip_contrato o tipo_contrato)
                              $tip_raw = $resultado['tip_contrato'] ?? ($resultado['tipo_contrato'] ?? '');
                              $tip_contrato = mb_strtolower(trim((string)$tip_raw), 'UTF-8');
                              // Reglas coherentes con el dashboard de facilitadores:
                              // - contratista  => Facilitador
                              // - planta/instructor => Instructor
                              // - cualquier otro/ vacío => Profesor (rol genérico)
                              if (in_array($tip_contrato, ['contratista'], true)) {
                                  $etiqueta_profesor = 'Facilitador';
                              } elseif (in_array($tip_contrato, ['planta','instructor'], true)) {
                                  $etiqueta_profesor = 'Instructor';
                              } else {
                                  $etiqueta_profesor = 'Profesor';
                              }
                            ?>
                    ...
                    <?php elseif ($filtro === 'estudiante'): ?>
                        ...
                        <div class="result-actions">
                            <?php $idDet = $resultado['aprendiz_id'] ?? ($resultado['id'] ?? ''); ?>
                            <button type="button" class="btn btn-primary btn-ver-detalles" data-id=<?= htmlspecialchars($idDet) ?>>Ver Detalles</button>
                        </div>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
