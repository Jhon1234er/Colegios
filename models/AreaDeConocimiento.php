...
class AreaDeConocimiento {
    ...
    public function guardar(array $data): bool {
        $stmt = $this->pdo->prepare("INSERT INTO cursos (codigo, denominacion, duracion, version, linea_tecnoacademia, nombre) VALUES (?,?,?,?,?,?)");
        $nombre = $data['nombre'] ?? $data['denominacion'] ?? '';
        $duracionRaw = $data['duracion'] ?? null;
        if ($duracionRaw !== null) {
            $digits = preg_replace('/\D+/', '', (string)$duracionRaw);
            $duracion = $digits !== '' ? (int)$digits : null;
        } else {
            $duracion = null;
        }
        return $stmt->execute([
    ...
    public function guardarMultiple(array $rows): int {
        ...
        try {
            ...
            foreach ($rows as $r) {
                ...
                $durRaw = $r['duracion'] ?? null;
                if ($durRaw !== null) {
                    $d = preg_replace('/\D+/', '', (string)$durRaw);
                    $dur = $d !== '' ? (int)$d : null;
                } else { $dur = null; }
    ...
    public function obtenerTodas(): array {
        $stmt = $this->pdo->query("SELECT * FROM cursos ORDER BY nombre");
    ...
    public function obtenerPaginado(int $limit, int $offset): array {
        $limit = max(1, $limit);
        $offset = max(0, $offset);
        $stmt = $this->pdo->prepare("SELECT * FROM cursos ORDER BY nombre LIMIT ? OFFSET ?");
    ...
    public function obtenerLineasDistinct(): array {
        $stmt = $this->pdo->query("SELECT DISTINCT linea_tecnoacademia AS linea FROM cursos WHERE linea_tecnoacademia IS NOT NULL AND linea_tecnoacademia <> '' ORDER BY linea");
    ...
    public function actualizar($id, array $data) {
        $stmt = $this->pdo->prepare("UPDATE cursos SET codigo=?, denominacion=?, duracion=?, version=?, linea_tecnoacademia=?, nombre=? WHERE id = ?");
        $nombre = $data['nombre'] ?? $data['denominacion'] ?? '';
        $duracionRaw = $data['duracion'] ?? null;
        if ($duracionRaw !== null) {
            $digits = preg_replace('/\D+/', '', (string)$duracionRaw);
    ...
    public function suspender(int $id): bool {
        try {
            ...
            $stmt = $this->pdo->prepare("UPDATE cursos SET estado='suspendida' WHERE id = ?");
            $stmt->execute([$id]);
    ...
    public function activar(int $id): bool {
        $stmt = $this->pdo->prepare("UPDATE cursos SET estado='activa' WHERE id = ?");
        return $stmt->execute([$id]);
    }
}
